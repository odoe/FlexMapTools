package net.odoe.FlexMapTools.components {
	...
	import net.odoe.FlexMapTools.components.events.IdentifyToolEvent;
	...
	[Event(name="identifyToolActive", type="net.odoe.FlexMapTools.components.events.IdentifyToolEvent")]
	/**
	...
	public class IdentifyTool extends SkinnableComponent implements IIdentifyTool {
		...
		private static const logger:ILogger = LogUtil.getLogger(IdentifyTool);
		...
		protected function iterateResults(results:Array):void {
			...
			for each (idResult in results) {
				...
				for (field in obj) {
					...
					// check if field has an asset id
					if (field == "CSDAssetID")
						assetID = obj[field];
						// check if field has number and needs formatting
					else if ((_field == "AREA" || _field == "LENGTH") && !isNaN(Number(obj[field]))) {
					...
					if (_field != "SHAPE_AREA" && _field != "SHAPE_LENG" &&
						...
						_field != "FID" && _field != "ARC_") {
						content += "<p><span fontWeight='bold'>" + _field + "</span>: " + obj[field] + "</p>";
							
					}
				}
				
				logger.debug("identify content = {0}", content);
		...
		protected function onButtonClicked_handler(event:MouseEvent):void {
			...
			idPoint = null;
			layerId = cbLayers.selectedItem.id;
			layerName = cbLayers.selectedItem.name;
			identifyLayer();
		}
		...
		protected function onCbLayersClose_handler(event:DropDownEvent):void {
			if (event.currentTarget.selectedItem) {
				...
				layerName = event.currentTarget.selectedItem.name;
				identifyLayer();
			}
		}
		...
		protected function sendParams():void {
			...
			logger.debug("Identify Task URL = {0}", idTask.url);
